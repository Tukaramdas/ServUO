...
namespace Server.Items.Crops
{
	...
	public class Buoy : BaseLobsterTrap
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
            if (from.Mounted && !TrapHelper.CanWorkMounted)
            {
                from.SendMessage( "You cannot pull the trap from the water when mounted!" );
                return;
            }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            switch (Utility.Random(6))
            {
                case 0: from.AddToBackpack(new EmptyLobsterTrap());
                    ...
                    this.Delete();break;
                case 1:
                    from.AddToBackpack(new EmptyLobsterTrap());
                    from.AddToBackpack(new Fish(Utility.Random(1, 4)));
                    from.AddToBackpack(new SpringDragonfish());
                    from.SendMessage("You remove fish from the trap and put it in your pack.");
                    this.Delete(); break;
                case 2:
