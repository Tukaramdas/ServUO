...
namespace Server.Items
{
	...
	{
		[Constructable]
		public AssemblyUpgradeKit( ) : base( 0x1505 )
		{
		...
		public AssemblyUpgradeKit( Serial serial ) : base( serial )
		{
		}
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( AssemblyUpgradeKit kit ) : base( 1, false, TargetFlags.Beneficial )
			{
				m_Kit = kit;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is BaseAssembly )
				{
					if ( ((BaseAssembly)targeted).ControlMaster != from )
						...
						from.SendMessage( "That assembly has already been upgraded as much as possible" );
					else
					{
						from.SendMessage( "You upgrade the assembly.  It appears to be stronger now." );
						m_Kit.Delete();
					}
				}
				else
