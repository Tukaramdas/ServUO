...
namespace Server.Mobiles
{
	public class MobileFeatures
	{
		private static string[] m_MobileFeatures = new string[]
					{
						"AntiArchery",	////Teleports to archers outside of 5 tile radius (chance 50%)//
		...
		private class BomberTimer : Timer
		{
			...
			public BomberTimer( Mobile m, Mobile from ) : base( TimeSpan.FromSeconds( 1.0 ) )
			{
				m_Mobile = m;
				m_From = from;
	  ...
	  { 
		...
		{ 
			public RobotSpamTimer() : base( TimeSpan.FromSeconds( 1.5 ) ) 
			{ 
	  ...
	  { 
		...
		{ 
			public HumanSpamTimer() : base( TimeSpan.FromSeconds( 1.5 ) ) 
			{ 
				Priority = TimerPriority.OneSecond;
			} 
		...
		public static void DoDisplace(BaseCreature mobile, Mobile player)
		{
			...
			mobile.Frozen = false;
			player.Combatant = null;
			Effects.SendLocationParticles( EffectItem.Create( mobile.Location, mobile.Map, EffectItem.DefaultDuration ), 0x3779, 50, 50, 5052 );
	  ...
	  { 
		...
		{ 
			public StonedSpamTimer() : base( TimeSpan.FromSeconds( 1.5 ) ) 
			{ 
				Priority = TimerPriority.OneSecond;
			} 
