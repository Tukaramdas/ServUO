...
namespace Server.Items
{
	...
	{
		[Constructable]
		public RunicClockworkAssembly( ) : base( 0x1eac )
		{
		...
		public RunicClockworkAssembly( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			powercore = ourPack.FindItemByType( typeof( RunicGolemPowerCore ) );
			if ( powercore != null )
			{
				ConstructRunicGolem( from );
				return;
			}
			...
			from.SendMessage( "You don't have any power cores in your backpack.  You cannot contruct anything." );
		}
		...
		public void ConstructGargoyle( Mobile from )
		{
			...
			assembly.Skills[SkillName.Anatomy].Base = Math.Round(assembly.Skills[SkillName.Anatomy].Base * percentage, 1);
			assembly.Skills[SkillName.Swords].Base = Math.Round(assembly.Skills[SkillName.Swords].Base * percentage, 1);
			assembly.Skills[SkillName.Tactics].Base = Math.Round(assembly.Skills[SkillName.Tactics].Base * percentage, 1);
			assembly.Skills[SkillName.MagicResist].Base = Math.Round(assembly.Skills[SkillName.MagicResist].Base * percentage, 1);
