...
{
	public class BaseFishingNet : Item, IChopable
	{
		public virtual bool CanSetNet{ get{ return true; } }
		...
		public BaseFishingNet( int itemID ) : base( itemID )
        {
        }
		...
		public BaseFishingNet( Serial serial ) : base( serial )
        {
        }
    ...
    public class NetHelper
	{
		...
		public static bool CheckCanNet( BaseFishingNet net, Map map, int x, int y )
		{
            if (net.CanSetNet && ValidateWater(map, x, y)) return true;
			return false;
		}
        ...
        public class CatchTimer : Timer
        {
            ...
            private int cnt;
            private double rnd;
            public CatchTimer(Item empty, Type nettype, Mobile fisher) : base(TimeSpan.FromSeconds(5), TimeSpan.FromSeconds(10))
            {
            ...
            protected override void OnTick()
            {
                if (cnt++ / 5 > rnd)
                {
                    if ((i_empty != null) && (!i_empty.Deleted))
                    {
                        ...
                        Item newitem = Activator.CreateInstance(t_net, args) as Item;
                        if (newitem == null )
                        {
        ...
        public static ArrayList CheckNet(Point3D pnt, Map map, int range)
		{
			ArrayList lnets = new ArrayList();
			IPooledEnumerable eable = map.GetItemsInRange( pnt, range );
			foreach ( Item lnet in eable )
            {
                if ((lnet != null) && (lnet is BaseFishingNet)) lnets.Add((BaseFishingNet)lnet);
            }
			eable.Free();
            return lnets;
		}

		public class Weeds : BaseFishingNet
		{
			...
			public override void OnDoubleClick(Mobile from)
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
                    if ((from == m_sower) || (DateTime.UtcNow >= planted.AddDays(3)))
					{
                        ...
                        EmptyFishingNet net = new EmptyFishingNet();
