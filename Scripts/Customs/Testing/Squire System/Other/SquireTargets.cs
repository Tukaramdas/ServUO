...
namespace Server.Mobiles
{
   ...
   // 
            ...
            protected override void OnTarget(Mobile from, object provokeone)
            {
                ...
                if (provokeone is BaseCreature && m_Squire.CanBeHarmful((Mobile)provokeone, true))
                {
                    ...
                    else if (creature.Controlled)
                    {
                        if (m_Squire.m_SquireBeQuiet == false)
                        {
                            SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.LoyalToTheirMaster, null, null);
                        }
                    }
                    else if (creature.IsParagon && BaseInstrument.GetBaseDifficulty(creature) >= 160.0)
                    {
                        if (m_Squire.m_SquireBeQuiet == false)
                        {
                            SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.NoChanceToProvoke, null, null);
                        }
                    }
                    else
                    {
                        ...
                        if (m_Squire.m_SquireBeQuiet == false)
                        {
                            SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.CantProvokeOne, null, null);
                        }
                ...
                else
                {
                    if (m_Squire.m_SquireBeQuiet == false)
                    {
                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.CantInciteAnger, null, null);
                    }
                }
            }
        }
            ...
            protected override void OnTarget(Mobile from, object provoketwo)
            {
                ...
                if (provoketwo is BaseCreature)
                {
                    ...
                    else if (m_Creature.Map != creature.Map || !m_Creature.InRange(creature, BaseInstrument.GetBardRange(m_Squire, SkillName.Provocation)))
                    {
                        if (m_Squire.m_SquireBeQuiet == false)
                        {
                            SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.TooFarApartToProvoke, null, null);
                        }
                    }
                    else if (m_Creature != creature)
                    {
                        m_Squire.NextSkillTime = Core.TickCount + 10000;
                        ...
                        if (m_Squire.CanBeHarmful(m_Creature, true) && m_Squire.CanBeHarmful(creature, true))
                        {
                            ...
                            {
                                m_Squire.NextSkillTime = Core.TickCount + 5000;
                                if (m_Squire.m_SquireBeQuiet == false)
                                {
                            ...
                            else
                            {
                                if (!m_Squire.CheckTargetSkill(SkillName.Provocation, creature, diff - 25.0, diff + 25.0))
                                {
                                    ...
                                    if (m_Squire.m_SquireBeQuiet == false)
                                    {
                                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.FailedPerformanceProvoke, null, null);
                                    }
                                    m_Instrument.PlayInstrumentBadly(m_Squire);
                                    m_Instrument.ConsumeUse(m_Squire);
                                }
                                else
                                {
                                    if (m_Squire.m_SquireBeQuiet == false)
                                    {
                                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.GoodPerformanceProvoke, null, null);
                                    }
                                    m_Instrument.PlayInstrumentWell(m_Squire);
                                    m_Instrument.ConsumeUse(m_Squire);
                    ...
                    else
                    {
                        if (m_Squire.m_SquireBeQuiet == false)
                        {
                            SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.ProvokeOnThemselves, null, null);
                        }
                    }
                }
                else
        ...
        internal class SquirePeacemakingTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (!(targeted is Mobile))
                {
                    if (m_Squire.m_SquireBeQuiet == false)
                    {
                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.CantCalm, null, null);
                    }
                }
                else if (from.Region.IsPartOf(typeof(Engines.ConPVP.SafeZone)))
                {
                    if (m_Squire.m_SquireBeQuiet == false)
                    {
                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.CantCalmHere, null, null);
                    }
                }
                else if (((Mobile)targeted).Region.IsPartOf(typeof(Engines.ConPVP.SafeZone)))
                {
                    if (m_Squire.m_SquireBeQuiet == false)
                    {
                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.CantCalmThere, null, null);
                    }
                }
                ...
                else
                {
                        ...
                        else
                        {
                            ...
                            if (map != null)
                            {
                                ...
                                if (!calmed)
                                    if (m_Squire.m_SquireBeQuiet == false)
                                    {
                                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.PeaceNobody, null, null);
                                    }
                                    else
                                    if (m_Squire.m_SquireBeQuiet == false)
                                    {
                                        SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.GoodPerformancePeace, null, null);
                                    }
                            }
                        }
                    }
                    else
                    {
                        ...
                        else if (targ is BaseCreature && ((BaseCreature)targ).Uncalmable)
                        {
                            if (m_Squire.m_SquireBeQuiet == false)
                            {
                                SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.NoChanceToCalm, null, null);
                            }
                            m_SetSkillTime = true;
                        }
                        else if (targ is BaseCreature && ((BaseCreature)targ).BardPacified)
                        {
                            if (m_Squire.m_SquireBeQuiet == false)
                            {
                                SquireDialog.DoSquireDialog(from, m_Squire, SquireDialogTree.AlreadyCalmed, null, null);
                            }
                            m_SetSkillTime = true;
                        }
        ...
        internal class SquireStealingTarget : Target
        {
            ...
            public SquireStealingTarget(Mobile from, Squire squire) : base(20, false, TargetFlags.None) //Changed 1.8.8 from base( 1, false, to base( 20, false to make it so you don't need to be next to the target
            {
                m_From = from;
                m_Squire = squire;
        ...
        internal class SquireLockpickTarget : Target
        {
            ...
            public SquireLockpickTarget(Squire squire, Lockpick item) : base(20, false, TargetFlags.None) //Changed 1.8.8 from base( 1, false, to base( 20, false to make it so you don't need to be next to the target
            {
                m_Item = item;
                m_Squire = squire;
            }
