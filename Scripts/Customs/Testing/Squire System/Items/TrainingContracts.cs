		...
		public override void OnDoubleClick( Mobile from )
		{
			if( from is Squire ) 
			{
				if( ((Squire)from).Skills[skill].Lock == SkillLock.Up )
				{
					...
					{
						((Squire)from).ControlMaster.SendMessage( "This squire knows more than this contract could teach them." );
					}
				}
				else
				{
				...
				if( o is Squire )
				{
					if( ((Squire)o).Controlled == true && ((Squire)o).ControlMaster == from )
					{
						if( ((Squire)o).Skills[skill].Lock == SkillLock.Up )
						{
							...
							{
								from.SendMessage( "This squire knows more than this contract could teach them." );
							}
						}
						else
						{
