...
namespace Server.Gumps
{
    public class ShowToolAnimal : Gump
    {
        ...
        public ShowToolAnimal(PlayerMobile owner) : base(0, 50)
        {
            ...
            AddLabel(0x16, 0x2c, 50, "Tito Tim's");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                ...
                pm.DoSpeech("*Protect me*", numArray, 0, 20);
                pm.Say("all guard me");
            }
            if (info.ButtonID == 2)
            {
                pm.CloseGump(typeof (ShowToolAnimal));
                pm.SendGump(new ShowToolAnimal(pm));
                pm.DoSpeech("*Follow me*", numArray2, 0, 20);
            ...
            if (info.ButtonID == 3)
            {
                pm.CloseGump(typeof (ShowToolAnimal));
                pm.SendGump(new ShowToolAnimal(pm));
                pm.DoSpeech("*All Drop*", numArray3, 0, 20);
                pm.Say("all drop");
            }
            if (info.ButtonID == 4)
            {
                pm.CloseGump(typeof (ShowToolAnimal));
                pm.SendGump(new ShowToolAnimal(pm));
               pm.DoSpeech("*All Kill*", numArray4, 0, 20);
                pm.Say("all kill");
            }
            if (info.ButtonID == 5)
            {
                pm.CloseGump(typeof (ShowToolAnimal));
                pm.SendGump(new ShowToolAnimal(pm));
               pm.DoSpeech("*All Stop*", numArray5, 0, 20);
                pm.Say("all stop");
            }
            if (info.ButtonID == 6)
            {
                pm.CloseGump(typeof (ShowToolAnimal));
                pm.SendGump(new ShowToolAnimal(pm));
              pm.DoSpeech("*All Stay*", numArray6, 0, 20);
                pm.Say("all stay");
            }
            if (info.ButtonID == 7)
            {
                pm.CloseGump(typeof (ShowToolAnimal));
                pm.SendGump(new ShowToolAnimal(pm));
                pm.DoSpeech("*All Come*", numArray7, 0, 20);
                pm.Say("all come");
            }
            if (info.ButtonID == 20)
            {
